  ...
  fetch(`${api_url}/${username}/repos`)
    ...
    .then((results) => {
      results.forEach((result) => {
        console.log(informationRepository(result));
      });
    });
  rl.close();
});
...
function informationUser(data) {
  return `
    ...
    ${chalk.red("  Location")}          : ${
    data.location ? data.location : "Empty"
  }
    ${chalk.red("  Followers")}         : ${data.followers}
    ${chalk.blue("  Following")}         : ${data.following}
      Bio               : ${data.bio ? `${data.bio}` : "Empty"}
    ${chalk.yellow("  Public Repository")} : ${
    data.public_repos ? data.public_repos : "Empty"
  }
    ${chalk.blue("  Twitter")}           : ${
    data.twitter_username ? data.twitter_username : "Empty"
  }
...
function informationRepository(data) {
  return `
    ...
    ${chalk.yellow("  Stars")}                : ${chalk.yellow(
